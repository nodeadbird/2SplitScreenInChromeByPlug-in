...
$('#open_url_new_tab').click(() => {
	...
	chrome.tabs.query({}, function (tabs) {
		...
		// alert(tabTitle[0]);
		// alert(tabTitle[1]);
		//默认将第一个第二个TAB页的URL传递过去
		var firstUrl = tabUrl[0];
		var secondUrl = tabUrl[1];
		var firstTitle = tabTitle[0];
		var secondTitle = tabTitle[1];
		chrome.tabs.create({ url: '../splitHtml.html?firstUrl=' + firstUrl + '&secondUrl=' + secondUrl + '&firstTitle=' + firstTitle + '&secondTitle=' + secondTitle });
	});
	//此方法只可以捕获到第一个TAB页面的URL传递过去
	// chrome.tabs.getSelected(null, function (tab) {
	// 	var tabUrl = tab.url;
	// 	if (tabUrl == 'chrome://newtab/') {
	// 		chrome.tabs.create({ url: '../splitHtml.html' });
	// 	} else {
	// 		chrome.tabs.create({ url: '../splitHtml.html?tabUrl=' + tabUrl });
	// 	}
	// });
	//chrome.tabs.create({url: 'http://192.168.1.106:8080/split'});
	//chrome.tabs.create({url: 'http://127.0.0.1:8000/split'});
});
...
// 打开手写批注
$('#openUrlCanvas').click(() => {
	//chrome.tabs.create({url: 'http://192.168.1.106:8080/canvas'});
	chrome.tabs.create({ url: 'http://127.0.0.1:8000/canvas' });
});
